...
import kr.or.ddit.vo.ProjectFileDetailVO;
...
//	String uploadPath = "C:/sts4-4.26.0/workspace/entProj/src/main/resources/static/upload/";
	...
	//프로젝트 업로드용													프로젝트 이름 넣기
													//파일객체,    프로젝트번호,        간트차트번호
	public long multiImageUpload(MultipartFile[] multipartFiles, String pro, int ganttChartsNo) {
		long fileGroupNo = 0L;
		int result = 0;
		...
		// + "\\" +
		// 2024\\08\\08
		File uploadPath = new File(this.uploadPath, "\\project\\"+pro);
		...
		projectFileVO.setGanttChartsNo(ganttChartsNo);


		//실행전 projectFileVO(proFileNo=0, .., ganttChartsNo=9...)
		log.info("multiImageUpload->projectFileVO(전) : " + projectFileVO);
		result += this.fileGroupMapper.insertProjectFile(projectFileVO);
		//실행후 projectFileVO(proFileNo=20250107008, .., ganttChartsNo=9...)
		log.info("multiImageUpload->projectFileVO(후) : " + projectFileVO);
		...
		for(MultipartFile multipartFile : multipartFiles) {
			...
			try {
				...
				ProjectFileDetailVO projectFileDetailVO = new ProjectFileDetailVO();
				projectFileDetailVO.setFileSn(++counter);							//복합키(P.K). 1, 			2, 			 3..
				//FILE_GROUP 테이블에 insert 시 FileGroupvO의 fileGroupNo는 채워져있음
				projectFileDetailVO.setProFileNo(projectFileVO.getProFileNo());	//복합키(P.K)  20241119004   20241119004   20241119004
				projectFileDetailVO.setFileOriginalName(multipartFile.getOriginalFilename());//개똥이.jpg
				projectFileDetailVO.setFileSaveName(fileName); //uuid_개똥이.jpg
				...
				projectFileDetailVO.setFileSaveLocate("/resources/upload/" +
						"project/" +
							pro + "/" + fileName
						);//웹경로
				projectFileDetailVO.setFileSize(fileSize);//bytes
				projectFileDetailVO.setFileExt(
						fileName.substring(fileName.lastIndexOf(".")+1)
						);//jpg
				projectFileDetailVO.setFileMime(contentType);
				projectFileDetailVO.setFileFancysize(
							makeFancySize(String.valueOf(fileSize))
						);//bytes->Mb. public String makeFancySize(String bytes) {
				projectFileDetailVO.setFileSaveDate(null);
				projectFileDetailVO.setFileDwldCnt(0);
				/*
				...
				result += this.fileGroupMapper.insertProjectDetail(projectFileDetailVO);

				//fileGroupNo : 20241119005
				log.info("result : " + result);
				log.info("projectFileDetailVO : " + projectFileDetailVO);

				fileGroupNo = projectFileVO.getProFileNo();
