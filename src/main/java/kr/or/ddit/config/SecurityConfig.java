...
import kr.or.ddit.controller.CheckAuthenticationSuccessHandler;
		...
		return http.csrf(csrf -> csrf.disable()).httpBasic(hbasic -> hbasic.disable())
				...
				.authorizeHttpRequests(
						authz -> authz.dispatcherTypeMatchers(DispatcherType.FORWARD, DispatcherType.ASYNC).permitAll() // forward					// 허가
								.requestMatchers("/login","/signup","/user", "/error", "/adminlte/**", "/js/**", "/ckeditor/**", "/css/**", "/images/**", "/upload/**", "/resources/**","/upload/**","/error/**","/","/assets/**",
										"/views/**","https://www.youtube.com/**","/WEB-INF/**","/main/**"
										,"/assets/**","/assets2/**","/assets3/**","/notice/**","/listAjax/**","/shop/**"
										,"/goods/**"
//										,"/**"
										).permitAll()
								.requestMatchers("/companypage/**").hasRole("COMPANY")
								.requestMatchers("/managepage/**").hasRole("MANAGER")
//								.requestMatchers("/manager/**").hasAnyRole("CEO","MANAGER")
								.anyRequest().authenticated())
				.formLogin(formLogin->formLogin
						.loginPage("/login")
						.defaultSuccessUrl("/")
	                    .successHandler(new CheckAuthenticationSuccessHandler())  // CustomAuthenticationSuccessHandler 인스턴스를 직접 생성하여 설정
