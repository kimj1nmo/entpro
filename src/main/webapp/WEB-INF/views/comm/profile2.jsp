...
.profile-section {
    ...
    align-items: center;
    margin: 30px 20px;
    justify-content: flex-start; /* 기본값으로 왼쪽 정렬 */
}
.profile-photo {
    width: 140px;
    height: 140px;
    border-radius: 50%;
    object-fit: cover;
    margin-right: 20px; /* 사진과 정보 사이의 간격을 추가 */
}
.artist-info {
    display: flex;
    flex-direction: column; /* 닉네임과 생일 정보를 세로로 쌓을 수 있도록 변경 */
    justify-content: center;
}
.artist-name-check {
    display: flex;
...
.follow-button {
    margin-left: auto; /* 팔로우 버튼을 오른쪽으로 이동 */
...
.tabs {
    margin-left: 20px; /* 탭 섹션을 왼쪽으로 정렬 */
    display: flex;
    margin: 20px 0;
}
.tabs button {
  ...
  <div class="container-fluid">
    ...
    <div class="profile-section">
        <c:forEach var="communityVO" items="${communityVOList}" varStatus="stat">
            <!-- Check if the memId matches and if the image is not already shown -->
            <c:if test="${communityVO.memId == param.memId && not fn:contains(shownImages, communityVO.proSaveLocate)}">
                <div>
                    <img class="profile-photo"
                         src="${pageContext.request.contextPath}${communityVO.proSaveLocate}"
                         alt="${fileDetailVO.fileOriginalName}"
                         title="${fileDetailVO.fileOriginalName}" />
                </div>
                <!-- Add the current image to the list of shown images -->
                <c:set var="shownImages" value="${shownImages},${communityVO.proSaveLocate}" scope="page" />
            </c:if>
        </c:forEach>
