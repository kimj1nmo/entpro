			...
			<div class="row mb-2">
				...
				<div class="col-sm-6">
					<ol class="breadcrumb float-sm-right">
						<li class="breadcrumb-item"><a href="/managepage/manage/list">프로젝트 관리</a></li>
			...
			<div class="row">
				<div class="col-12">
					<div class="card">
									...
									<c:forEach var="departmentVO" items="${departmentVOList}">
										<tr>
											<td><input type="checkbox" class="deleteCheckbox"
													data-emp-list-id="${departmentVO.memId}" value="${departmentVO.departNo}"></td>
						...
						<!-- 페이징 및 버튼 영역 -->
						<div id="divMove" class="card-footer2 d-flex justify-content-end align-items-center">
							<!-- <div id="divPagingArea" style="background-color: white; flex-grow: 1;"></div> -->
							<div class="d-flex" style="gap: 15px; padding-right: 15px;">
								<button id="empReg" data-toggle="modal" data-target="#modal-lg"
									class="btn btn-primary">등록</button>
								<button id="deleteEmp" type="button" class="btn btn-danger">삭제</button>
							</div>
						</div>
					</div>
				</div>
...
<script>
		...
		$.ajax({
			...
			success: function (articlePage) {
				...
				$.each(articlePage.content, function (idx, departmentVO) {
					...
					<td><i class="fas fa-calendar-check" onclick="location.href='/managepage/manage/gantt/\${proNo}'" style="cursor: pointer;">&nbsp;직원 일정 확인</i></td>
				</tr>`;	// end each
	...
	$("#deleteEmp").on("click", function () {
		$(".deleteCheckbox:checked").each(function () {
			delBox.push({ memId: $(this).data("empListId") });
		})
