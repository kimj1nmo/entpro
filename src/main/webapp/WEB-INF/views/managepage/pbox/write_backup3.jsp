...
<div id="pageTtl" class="card-header1" >
	<div style=" width: 120vh; display: flex; justify-content: left; align-items: space-between;">
		<h2 id="pageTtlCn" style="margin: 50px; margin-left: 0; margin-top: 0; width: 120vh; display: flex; justify-content: left; align-items: space-between;"><b>   기안하기 ${memberVO}</b></h2>
...
<table class="appr_table" style="width: 120vh;">
	<tr>
		...
		<td id="team">${memberDetail.departName}</td>
		<td id="team"></td>
		<td id="team"></td>
		<td id="team"></td>
		<td id="team"></td>
	...
	<tr>
		<td id="team">${memberDetail.posName}</td>
		<td id="team"></td>
		<td id="team"></td>
		<td id="team"></td>
		<td id="team"></td>
	...
	<tr>
		<td id="name">${memberVO.memName}</td>
		<td id="name"></td>
		<td id="name"></td>
		<td id="name"></td>
		<td id="name"></td>
...
<div id="right">
	<div id="aplTextarea">
		<table >
			<tr style="text-align: center;" class="clsTr">
...
$("#saveBtn").click(function () {
	...
	$(".clsTr").each(function(idx,trs){
		console.log("saveBtn->trs : ", trs);
		let tds = $(this).children("td");//1	결재	시장조사팀	황보리(팀장)

				console.log("tds[0] : ", tds[0].innerHTML);//1
				console.log("tds[1] : ", tds[1].innerHTML);//결재
				console.log("tds[2] : ", tds[2].innerHTML);//시장조사팀
				console.log("tds[3] : ", tds[3].innerHTML);//황보리(팀장)
				console.log("tds[4] : ", tds[4].innerHTML);//아이디

				/*
				const departName = item.departName;//부서
				const nameWithPosition = item.name; // e.g., "홍길동(과장)"
				*/
				rowData = {
					// atrzManNo: row.find('td').eq(0).text(),
					atrzManSeq : (idx+1),
					atrzManId : tds[4].innerHTML,	
					name : tds[3].innerHTML,
					departName : tds[2].innerHTML,
					// atrzManDate :SYSDATE
	...
	console.log("JSON 데이터->data : ", data);


	// 테이블 row 요소
	const teamRow = $(".appr_table tr").eq(0).find("td").slice(2);
	const posRow = $(".appr_table tr").eq(1).find("td").slice(1);
	const nameRow = $(".appr_table tr").eq(3).find("td[id='name']").slice(1);
	const pRow = $("#ref_p");
	...
	let teamIndex = -1; // 결재 데이터를 삽입할 위치
	// 참조 데이터 저장 배열 (중복 방지용)
	let selectedRefs = [];
	...
	data.forEach((item) => {
		 console.log("item.atrzManType : ", item.atrzManType,", teamIndex : ", teamIndex,", teamRow.length : ",teamRow.length);
...
function tableToJson() { 
	...
	tableRows.each(function () {
		...
		const rowData = {
			// atrzManNo: row.find('td').eq(0).text(),
			atrzManSeq : row.find('td').eq(0).text(),
	...
	const jsonData = tableToJson(); // JSON 변환
	// 데이터 전송
	axios.post('/managepage/pbox/write/savedoc', {
		atrzDocNm: atrzDocNm,
		atrzDocCn: atrzDocCn,
		...
		apprManList : jsonData
	})
	.then(response => {
		// console.log("서버 응답:", response);
    const docNo = response.data;  // 서버에서 반환된 문서 번호
    if (docNo > 0) {
        // 상세 페이지로 리다이렉트
        window.location.href = "/pbox/detail/" + docNo;
    } else {
        console.error("저장 실패");
    }
	})
